...
st.set_page_config(
    layout="wide",
    page_title="학생 행동특성 생성기 (다수 학생)",
    initial_sidebar_state="collapsed"  # 사이드바를 초기에 접힌 상태로 설정
)
st.title("📝 학생 행동 특성 및 종합의견 생성 도우미 (초등 - 다수 학생 지원)")
st.markdown("Gemini API를 활용하여 여러 학생의 특성에 맞는 종합의견 초안을 생성하고 Excel 파일로 저장합니다.")

# --- 사이드바 설정 ---
# 사이드바 내용은 initial_sidebar_state가 "collapsed"여도 사용자가 열면 보이게 됩니다.
with st.sidebar: # 사이드바 관련 UI는 st.sidebar 블록 안에 명시적으로 배치
    st.header("⚙️ 설정")
    retrieved_api_key = get_api_key() 
    st.markdown("[Google AI Studio 바로가기](https://aistudio.google.com/app/apikey)")
    num_students = st.number_input("생성할 학생 수를 입력하세요:", min_value=1, value=1, step=1)
    st.info("`pandas`와 `openpyxl` 라이브러리가 필요합니다. 설치되지 않은 경우 터미널에서 `pip install pandas openpyxl`을 실행해주세요.")
...
st.markdown("---")
st.caption("본 앱은 금일초등학교 교사 강승현이 제작하였습니다. 문의사항은 https://litt.ly/4sh.space에 문의해주세요.")
